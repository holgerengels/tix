...
router.post('/tickets', verifyToken, async (req, res) => {
    try {
        ...
        if (wf) {
            ...
            const validationResult = await validateTicket(req.body, wf, null, req.user);
...
router.post('/tickets/:id/action', verifyToken, async (req, res) => {
    try {
        ...
        const { validateTicket } = require('./validation');
        const validationResult = await validateTicket(ticket.toObject(), currentWf, null, req.user);
        if (!validationResult.isValid) {
            return res.status(400).json({
                message: `Validation failed: ${validationResult.errors.join(', ')}`
            });
